...
module memory_ctrl(
		...
		//	stall signal
		output reg memctrl_stallreq
    );

	reg[`CacheLineBits-1:0] icache[`CacheLineNum];
	reg[`CacheLineBits-1:0] dcache[`CacheLineNum];
	reg inst_stallreq;
	reg data_stallreq;

	assign memory_ctrl = inst_stallreq | data_stallreq;
	...
	uart_comm uart0(clk, rst, send_flag, send_data, recv_flag, recv_data, 
		sendable, receivable, Tx, Rx);
